...
use server::util::{point_in_triangle_bary, lin_map, orientation, point_in_triangle};
...
impl EdgeGuesser {
    ...
    fn build(
        ...
        for (x, guess_row) in guesses.iter_mut().enumerate() {
            for (y, guess) in guess_row.iter_mut().enumerate() {
                let xp = lin_map(0., 256., domain.0.start, domain.0.end, x as f64);
                let yp = lin_map(0., 256., domain.1.start, domain.1.end, y as f64);
                *guess = find_triangle(vertices, triangulation, &[xp, yp], last_guess, i64::MAX);
                last_guess = *guess * 3;
            }
    ...
    fn guess(&self, p: &[f64; 2]) -> usize {
        let [x, y] = p;
        let xi = lin_map(self.domain.0.start, self.domain.0.end,  0., 256., *x).round() as usize;
        let yi = lin_map( self.domain.1.start, self.domain.1.end, 0., 256., *y).round() as usize;
...
fn find_triangle(
    ...
    loop {
        ...
        limit -= 1;
        // TODO: might need this logic, so keeping it in as a comment
